...
var (
	QuerySelectExpiredConsents = dbmodel.DBQuery{
		...
		Query:         "SELECT CONSENT_ID FROM CONSENT WHERE VALIDITY_TIME < ? AND CURRENT_STATUS = ?",
		PostgresQuery: "SELECT CONSENT_ID FROM CONSENT WHERE VALIDITY_TIME < $1 AND CURRENT_STATUS = $2",
	}
	...
	QueryExpireConsentAuthResources = dbmodel.DBQuery{
		...
		Query:         "UPDATE CONSENT_AUTH_RESOURCE SET AUTH_STATUS = ?, UPDATED_TIME = ? WHERE CONSENT_ID = ? AND AUTH_STATUS = ?",
		PostgresQuery: "UPDATE CONSENT_AUTH_RESOURCE SET AUTH_STATUS = $1, UPDATED_TIME = $2 WHERE CONSENT_ID = $3 AND AUTH_STATUS = $4",
	}
)
